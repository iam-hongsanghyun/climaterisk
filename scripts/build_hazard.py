"""Build the local hazard catalog (the platform's CLIMADA-ready perils database).

Runs in the CLIMADA conda env. Two ways to add a hazard:

  # 1. Convert a standardized observation grid (from real ingestion) -> catalog HDF5
  python scripts/build_hazard.py convert path/to/grid.json

  # 2. Cache a CLIMADA Data API hazard for offline / reproducible runs
  python scripts/build_hazard.py cache \\
  ...
  python scripts/build_hazard.py list
"""
...
from climaterisk_worker import catalog  # noqa: E402
from climaterisk_worker.hazard_convert import convert_grid_to_catalog  # noqa: E402
...
def cmd_ingest(args: argparse.Namespace) -> None:
    """Run an in-UI ingest refiner from the CLI (same code path as the worker)."""
